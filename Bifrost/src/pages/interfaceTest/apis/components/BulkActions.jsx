...
import { deleteApiByIds, deactivateApiByIds, activateApiByIds } from '@/services/backend/apis';
...
const BulkActions = (props) => {
  ...
  const bulkDeactivate = async () => {
    ...
    const ids = keys.join(',');
    const success = await deactivateApiByIds(ids);
    if (success) {
  ...
  const bulkActivate = async () => {
    ...
    const keys = props.values.selectedRowKeys;
    const ids = keys.join(',');
    const success = await activateApiByIds(ids);
    if (success) {
      const msg = `${intl.formatMessage({ id: 'pages.caseMaintain.dashboard.actions.bulk', })}${intl.formatMessage({ id: 'pages.caseMaintain.dashboard.actions.deactivate', })}${intl.formatMessage({ id: 'ui.msg.caseMaintain.dashboard.actions.result.success', })}`;
      message.success(msg);
      props.values.onCleanSelected();
  ...
  const bulkDelete = async () => {
    const keys = props.values.selectedRowKeys;
    const ids = keys.join(',');
    const success = await deleteApiByIds(ids);
    if (success) {
