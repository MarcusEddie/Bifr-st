...
var _default = /*#__PURE__*/_asyncToGenerator(function* () {
  if (!cache || process.env.NODE_ENV === 'test') {
    ...
    const components = _fs().default.existsSync(builtinPath) ? _fs().default.readdirSync(builtinPath) // filter .js/.ts/.jsx/.tsx, and exclude .d.ts
    .filter(file => /((?<!\.d)\.ts|\.(jsx?|tsx))$/.test(file)).map(file => ({
